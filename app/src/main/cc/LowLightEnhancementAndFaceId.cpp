...
LowLightEnhancementAndFaceId::LowLightEnhancementAndFaceId(const void* model_data, size_t model_size,
                                                           bool use_gpu) {
  // Load the model
  ...
  // Choose CPU or GPU
  if (use_gpu) {
    delegate_ = TfLiteGpuDelegateV2Create(nullptr);
...
LowLightEnhancementAndFaceId::~LowLightEnhancementAndFaceId() {
...
bool LowLightEnhancementAndFaceId::IsInterpreterCreated() {
  if (!interpreter_) {
    return false;
  } else {
    return true;
...
std::unique_ptr<int[]> LowLightEnhancementAndFaceId::DoLowLightEnhancement(int* ll_img_rgb) {
  // Allocate tensors and populate the input tensor data
  TfLiteStatus status = TfLiteInterpreterAllocateTensors(interpreter_);
  if (status != kTfLiteOk) {
    __android_log_print(ANDROID_LOG_INFO, "tflite ", " inside DoLowLightEnhancement 1");
  ...
  for (int i = 0, j = 0; i < kNumberOfInputPixels; i++) {
    // Alpha is ignored
    input_buffer[j++] = static_cast<float>((ll_img_rgb[i] >> 16) & 0xff) / 255.0;
    input_buffer[j++] = static_cast<float>((ll_img_rgb[i] >> 8) & 0xff) / 255.0;
    input_buffer[j++] = static_cast<float>((ll_img_rgb[i]) & 0xff) / 255.0;
  }
  ...
  if (status != kTfLiteOk) {
    __android_log_print(ANDROID_LOG_INFO, "tflite ", " inside DoLowLightEnhancement 2");
  ...
  if (status != kTfLiteOk) {
    __android_log_print(ANDROID_LOG_INFO, "tflite ", " inside DoLowLightEnhancement 3");
  ...
  if (status != kTfLiteOk) {
    __android_log_print(ANDROID_LOG_INFO, "tflite ", " inside DoLowLightEnhancement 4");
